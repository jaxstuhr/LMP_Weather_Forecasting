...
build_weather_data_frame = function(file_paths, dates) {
  
  # if different number of file paths and dates, throw an error
  if (length(file_paths) != length(dates)) {
    return(NA)
  }
  
  # initialize complete weathere data frame
  weather_data_frame = data.frame(matrix(nrow = 0, ncol = 6))
  colnames(weather_data_frame) = c("value", "date", "hour", "parameter", "lat", "lon")
...
full_matrix = build_weather_data_frame(ac_weather_files[1:20], dates = c(1:20))
  ...
  for (lmp_file in file_paths) {
    ...
    for (i in 2:length(current_node_lmps)) {
      ...
      current_day_lmps = data.frame(
        ...
        lmp = lmps_list,
        # date from current data
        date = names(current_node_lmps[i]),
...
lmp_data = build_lmp_data_frame(ac_lmp_files[1:2])
```
