---
title: "Node Location Document Scraping"
author: "Jaxon Stuhr"
...
# various text parsing to split out region, lat, and long
node_locations = raw_node_locations %>% 
  ...
  separate(col = leftover, into = c("lat", "long"), sep = "-",  extra = 'merge') %>% 
  separate(col = long, into = c("long1", "long2", "leftover"), sep = "\\.", extra = 'merge') %>% 
  mutate(long = str_c(long1, long2, sep = ".")) %>% 
  select(region, lat, long, leftover) %>% 
  ...
  select(region, lat, long, node_id) %>% 
  na.omit()
# convert lat, long to numerics
node_locations = node_locations %>% 
  mutate(lat = as.numeric(lat)) %>% 
  mutate(long = -1*as.numeric(long)) 
```
...
mapview(ca_nodes, xcol = "long", ycol = "lat", label = "node_id" , legend = FALSE, crs = 4269, grid = FALSE)
```
...
sample = raw_node_locations[1:20,] %>% 
  separate(col = raw_data, into = c("area", "leftover"), sep = "Node") %>% 
  separate(col = leftover, into = c("region", "leftover"), sep = "(?<=[A-Za-z])(?=[0-9])", extra = 'merge') %>% 
  separate(col = leftover, into = c("lat", "long", "leftover"), sep = "-",  extra = 'merge') %>% 
  separate(col = long, into = c("long1", "long2", "long_drop"), sep = "\\.", extra = 'merge') %>% 
  mutate(long = str_c(long1, long2, sep = ".")) %>% 
  select(region, lat, long, leftover) %>% 
  mutate(leftover = str_sub(leftover, -max(node_ids$id_length), -1)) %>% 
  mutate(node_id = NA)
